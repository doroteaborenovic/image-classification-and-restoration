# ==============================================================================
# ABLACIJA 5: BEZ DAMAGE ATTENTION MODULA (w/o Damage Attention Module)
...
# ==============================================================================
# ARHITEKTURA MODELA (BEZ DAMAGE ATTENTION MODULA)
# ==============================================================================
...
# DEKODER BEZ MAPE OŠTEĆENJA (nema kanala +1 za damage_map)
...
# GLAVNI MODEL ZA ABLACIJU 5: BEZ DAMAGE ATTENTION MODULA
class Restauracija_NoDamageAttention(nn.Module):
    def __init__(self, in_channels: int = 3, out_channels: int = 3, base_ch: int = 32):
        ...
        # NEMA DAMAGE ATTENTION MODULA - direktna rafinacija bottleneck-a
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # BEZ DAMAGE ATTENTION: fused tenzor ide direktno u bottleneck
        bottleneck_out = self.bottleneck_refine(fused)
        ...
        d4 = self.decoder4(bottleneck_out, sk4)
...
# ==============================================================================
model_no_dmg = Restauracija_NoDamageAttention(base_ch=32).to(device)
...
CKPT_SEPIA = os.path.join(DIR_ABLACIJA_CKPT, "ablation_no_damageattention_sepia25ep.pth")
if os.path.exists(CKPT_SEPIA):
    print(f"✓ [Keš] Učitavam postojeći checkpoint sa 25 epoha Sepia: {CKPT_SEPIA}")
    model_no_dmg.load_state_dict(torch.load(CKPT_SEPIA, map_location=device))
else:
    print(f"\n=======================================================")
    print(f" FAZA 1: Trening bez Damage Attention ({EPOCHS_SEPIA} epoha na Sepia)")
    print(f"=======================================================")
    opt_pretrain = torch.optim.AdamW(model_no_dmg.parameters(), lr=LR_PRETRAIN, weight_decay=1e-4)
    ...
    torch.save(model_no_dmg.state_dict(), CKPT_SEPIA)
    print(f"✓ Sačuvan bazni Sepia checkpoint: {CKPT_SEPIA}")

# FAZA 2: 5 EPOHA FINE-TUNINGA NA TRENING SKUPU
CKPT_FINAL = os.path.join(DIR_ABLACIJA_CKPT, "ablation_no_damageattention_final.pth")
if os.path.exists(CKPT_FINAL):
    print(f"✓ [Keš] Učitavam finalni dotrenirani model: {CKPT_FINAL}")
    model_no_dmg.load_state_dict(torch.load(CKPT_FINAL, map_location=device))
else:
    print(f"\n=======================================================")
    print(f" FAZA 2: Fine-Tuning ({EPOCHS_FT} epoha na ciljnom datasetu)")
    print(f"=======================================================")
    opt_ft = torch.optim.AdamW(model_no_dmg.parameters(), lr=LR_FINETUNE, weight_decay=1e-4)
    ...
    torch.save(model_no_dmg.state_dict(), CKPT_FINAL)
    print(f"✓ Sačuvan finalni model: {CKPT_FINAL}")

# FAZA 3: EVALUACIJA NA VALIDACIJI
print(f"\n[INFO] Evaluacija modela (w/o Damage Attention) na validaciji...")
model_no_dmg.eval()
...
with torch.no_grad():
    for fname in val_files:
        ...
        out_t = torch.clamp(model_no_dmg(d_t), 0.0, 1.0)
...
print("\n" + "="*80)
print("REZULTAT EVALUACIJE: ABLACIJA 5 (w/o Damage Attention)")
print("="*80)
print(tabulate(rezultati, headers=["Konfiguracija", "PSNR [↑]", "SSIM [↑]", "LPIPS [↓]"], tablefmt="fancy_grid"))

csv_out = os.path.join(DRIVE_PROJECT_DIR, "rezultat_ablacija_5_no_damageattention.csv")
