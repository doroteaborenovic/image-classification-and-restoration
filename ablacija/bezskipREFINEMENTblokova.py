# ==============================================================================
# ABLACIJA 8: BEZ SKIP REFINEMENT BLOKOVA (w/o Skip Refinement Blocks)
...
# ==============================================================================
# ARHITEKTURA MODELA (BEZ SKIP REFINEMENT BLOKOVA)
# ==============================================================================
...
# GLAVNI MODEL ZA ABLACIJU 8: BEZ SKIP REFINEMENT BLOKOVA
class Restauracija_NoSkipRefine(nn.Module):
    def __init__(self, in_channels: int = 3, out_channels: int = 3, base_ch: int = 32):
        ...
        # NEMA Skip Refinement blokova (skip_refine1, 2, 3, 4 su uklonjeni)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # BEZ SKIP REFINE: Gejtovani skips se direktno prosleđuju dekoderu (bez Dense + Spectral rafinacije)
...
# ==============================================================================
model_no_skip_refine = Restauracija_NoSkipRefine(base_ch=32).to(device)
...
CKPT_SEPIA = os.path.join(DIR_ABLACIJA_CKPT, "ablation_no_skiprefine_sepia25ep.pth")
if os.path.exists(CKPT_SEPIA):
    print(f"✓ [Keš] Učitavam postojeći checkpoint sa 25 epoha Sepia: {CKPT_SEPIA}")
    model_no_skip_refine.load_state_dict(torch.load(CKPT_SEPIA, map_location=device))
else:
    print(f"\n=======================================================")
    print(f" FAZA 1: Trening bez Skip Refine ({EPOCHS_SEPIA} epoha na Sepia)")
    print(f"=======================================================")
    opt_pretrain = torch.optim.AdamW(model_no_skip_refine.parameters(), lr=LR_PRETRAIN, weight_decay=1e-4)
    for ep in range(EPOCHS_SEPIA):
        model_no_skip_refine.train()
        ep_loss = 0.0
        for d_t, c_t, _ in sepia_loader:
            d_t, c_t = d_t.to(device), c_t.to(device)
            opt_pretrain.zero_grad()
            with torch.amp.autocast('cuda'):
                pred = model_no_skip_refine(d_t)
    ...
    torch.save(model_no_skip_refine.state_dict(), CKPT_SEPIA)
    print(f"✓ Sačuvan bazni Sepia checkpoint: {CKPT_SEPIA}")

# FAZA 2: 5 EPOHA FINE-TUNINGA NA TRENING SKUPU
CKPT_FINAL = os.path.join(DIR_ABLACIJA_CKPT, "ablation_no_skiprefine_final.pth")
if os.path.exists(CKPT_FINAL):
    print(f"✓ [Keš] Učitavam finalni dotrenirani model: {CKPT_FINAL}")
    model_no_skip_refine.load_state_dict(torch.load(CKPT_FINAL, map_location=device))
else:
    print(f"\n=======================================================")
    print(f" FAZA 2: Fine-Tuning ({EPOCHS_FT} epoha na ciljnom datasetu)")
    print(f"=======================================================")
    opt_ft = torch.optim.AdamW(model_no_skip_refine.parameters(), lr=LR_FINETUNE, weight_decay=1e-4)
    ...
    torch.save(model_no_skip_refine.state_dict(), CKPT_FINAL)
    print(f"✓ Sačuvan finalni model: {CKPT_FINAL}")

# FAZA 3: EVALUACIJA NA VALIDACIJI
print(f"\n[INFO] Evaluacija modela (w/o Skip Refinement Blocks) na validaciji...")
model_no_skip_refine.eval()
...
with torch.no_grad():
    for fname in val_files:
        ...
        out_t = torch.clamp(model_no_skip_refine(d_t), 0.0, 1.0)
...
print("\n" + "="*80)
print("REZULTAT EVALUACIJE: ABLACIJA 8 (w/o Skip Refine)")
print("="*80)
print(tabulate(rezultati, headers=["Konfiguracija", "PSNR [↑]", "SSIM [↑]", "LPIPS [↓]"], tablefmt="fancy_grid"))

csv_out = os.path.join(DRIVE_PROJECT_DIR, "rezultat_ablacija_8_no_skiprefine.csv")
