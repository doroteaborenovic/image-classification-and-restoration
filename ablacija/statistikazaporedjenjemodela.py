...
class Restauracija(nn.Module):
    def __init__(self, in_channels: int = 3, out_channels: int = 3, base_ch: int = 32):
        ...
        # Ispravljene projekcione dimenzije
...
print(f"[INFO] Izabrano je tačno {len(val_files)} slika za validaciju i uporednu statistiku (N = 160).")

if os.path.exists(ADAPTED_CKPT_PATH):
    print(f"✓ [KEŠ] Učitavam postojeći adaptirani model (5 epoha): {ADAPTED_CKPT_PATH}")
    ckpt = torch.load(ADAPTED_CKPT_PATH, map_location=device, weights_only=False)
    sd = ckpt['model_state_dict'] if isinstance(ckpt, dict) and 'model_state_dict' in ckpt else ckpt
    moj_model.load_state_dict(sd)
elif os.path.exists(ALT_ADAPTED_CKPT_PATH):
    print(f"✓ [KEŠ] Učitavam postojeći adaptirani model (5 epoha): {ALT_ADAPTED_CKPT_PATH}")
    ckpt = torch.load(ALT_ADAPTED_CKPT_PATH, map_location=device, weights_only=False)
    sd = ckpt['model_state_dict'] if isinstance(ckpt, dict) and 'model_state_dict' in ckpt else ckpt
    moj_model.load_state_dict(sd)
else:
...
# ==============================================================================
# ZVANIČNI MICROSOFT MODEL (BOPBL PIPELINE)
# ==============================================================================
...
# ==============================================================================
print(f"\n[INFO] Računanje metrika (PSNR, SSIM, LPIPS) na validacionom skupu od {len(val_files)} slika...")
...
with torch.no_grad():
    for idx, fname in enumerate(val_files):
        ...
        if (idx + 1) % 20 == 0 or (idx + 1) == len(val_files):
...
df_per_image_all.to_csv(os.path.join(DRIVE_PROJECT_DIR, "per_image_bopbl_poredjenje_160slika.csv"))
print(f"\n✓ Sačuvane per-image metrike: {os.path.join(DRIVE_PROJECT_DIR, 'per_image_bopbl_poredjenje_160slika.csv')}")
...
# Agregacija kroz svih 5000 iteracija (čiste srednje vrednosti)
m_in_p = np.mean(all_runs_in_p)
m_in_s = np.mean(all_runs_in_s)
m_in_l = np.mean(all_runs_in_l)

m_moj_p = np.mean(all_runs_moj_p)
m_moj_s = np.mean(all_runs_moj_s)
m_moj_l = np.mean(all_runs_moj_l)

m_ms_p = np.mean(all_runs_ms_p)
m_ms_s = np.mean(all_runs_ms_s)
m_ms_l = np.mean(all_runs_ms_l)

# Statistički upareni testovi (N = 160)
...
print("\n" + "█" * 120)
print(f"  TABELA: NAUČNO POREĐENJE RESTAURACIJE (MICROSOFT BOPBL | N = {len(df_moj)})")
print("█" * 120)
print(tabulate(tabela_poredjenje, headers=zaglavlja, tablefmt="fancy_grid", stralign="center", numalign="center"))

csv_izlaz = os.path.join(DRIVE_PROJECT_DIR, "tabela_bopbl_direktno_poredjenje_160slika.csv")
