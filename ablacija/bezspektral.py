# ==============================================================================
# ABLACIJA 2: BEZ SPECTRAL ENCODER GRANE (w/o Spectral Encoder Stream) - FINALNO
# LOSS: Originalni RestauracijaLoss (FP32 stabilan + Multi-scale AUX Supervision)
# Protokol: 25 Epoha (Sepia Dataset) + 5 Epoha Fine-Tuning (Ciljni Skup) + Eval
# ==============================================================================
...
class RestorationDataset(Dataset):
    def __init__(self, root_dir=None, clean_dir=None, degraded_dir=None, img_size=256, train=False):
        ...
        # Slučaj 1: Ako postoje folderi '0' (clean) i '1' (degraded)
        if root_dir:
        ...
        # Slučaj 2: Eksplicitno prosleđeni folderi clean i degraded
...
# ==============================================================================
# MODEL: ABLACIJA 2 (BEZ SPECTRAL ENCODER GRANE)
# ==============================================================================
...
class DecoderRestorationBlock_NoSpectral(nn.Module):
...
class Restauracija_NoSpectral(nn.Module):
    def __init__(self, in_channels: int = 3, out_channels: int = 3, base_ch: int = 32):
        ...
        self.decoder4 = DecoderRestorationBlock_NoSpectral(base_ch * 8, base_ch * 8, base_ch * 4)
        self.decoder3 = DecoderRestorationBlock_NoSpectral(base_ch * 4, base_ch * 4, base_ch * 2)
        self.decoder2 = DecoderRestorationBlock_NoSpectral(base_ch * 2, base_ch * 2, base_ch)
        self.decoder1 = DecoderRestorationBlock_NoSpectral(base_ch, base_ch, base_ch)
...
sepia_ds = RestorationDataset(root_dir=LOCAL_SEPIA_DIR, img_size=IMG_SIZE, train=True)
...
CKPT_SEPIA = os.path.join(DIR_ABLACIJA_CKPT, "ablation2_no_spectral_sepia25ep_stable.pth")
CKPT_FINAL = os.path.join(DIR_ABLACIJA_CKPT, "ablation2_no_spectral_final_stable.pth")

# FAZA 1: 25 EPOHA NA SEPIA
if os.path.exists(CKPT_SEPIA):
    print(f"✓ [Keš] Učitavam postojeći checkpoint sa 25 epoha: {CKPT_SEPIA}")
    ...
    torch.save(model_no_spectral.state_dict(), CKPT_SEPIA)
...
if os.path.exists(CKPT_FINAL):
    ...
    torch.save(model_no_spectral.state_dict(), CKPT_FINAL)
    print(f"✓ Sačuvan finalni model: {CKPT_FINAL}")
else:
    torch.save(model_no_spectral.state_dict(), CKPT_FINAL)

# FAZA 3: EVALUACIJA NA VALIDACIONOM SKUPU
print(f"\n[INFO] Evaluacija modela (w/o Spectral Encoder)...")
model_no_spectral.eval()
...
with torch.no_grad():
    for d_t, c_t, _ in val_loader:
        d_t, c_t = d_t.to(device), c_t.to(device)
        out_t = torch.clamp(model_no_spectral(d_t), 0.0, 1.0)
...
print("\n" + "="*80)
print("REZULTAT EVALUACIJE: ABLACIJA 2 (w/o Spectral Encoder)")
print("="*80)
