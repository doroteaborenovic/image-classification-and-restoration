# ==============================================================================
# ABLACIJA 7: BEZ GATED SKIP CONNECTIONS (w/o Gated Skips) - FINALNO
...
# ==============================================================================
# MODEL: ABLACIJA 7 (BEZ GATED SKIP CONNECTIONS)
# ==============================================================================
...
class Restauracija_NoGatedSkips(nn.Module):
    def __init__(self, in_channels: int = 3, out_channels: int = 3, base_ch: int = 32):
        ...
        # NEMA GatedSkipConnection modula (direktan skip)
...
if os.path.exists(CKPT_SEPIA):
    ...
    for ep in range(EPOCHS_SEPIA):
        ...
        for batch_idx, (d_t, c_t, _) in enumerate(sepia_loader):
            ...
            if (batch_idx + 1) % ACCUMULATION_STEPS == 0 or (batch_idx + 1) == len(sepia_loader):
                torch.nn.utils.clip_grad_norm_(model_no_gated_skips.parameters(), max_norm=1.0)
                opt_pretrain.step()
                opt_pretrain.zero_grad()
    ...
    torch.save(model_no_gated_skips.state_dict(), CKPT_SEPIA)
...
if os.path.exists(CKPT_FINAL):
    ...
    torch.save(model_no_gated_skips.state_dict(), CKPT_FINAL)
    print(f"✓ Sačuvan finalni model: {CKPT_FINAL}")
else:
    torch.save(model_no_gated_skips.state_dict(), CKPT_FINAL)

# FAZA 3: EVALUACIJA NA VALIDACIJI
print(f"\n[INFO] Evaluacija modela (w/o Gated Skip Connections)...")
model_no_gated_skips.eval()
...
with torch.no_grad():
    for d_t, c_t, _ in val_loader:
        d_t, c_t = d_t.to(device), c_t.to(device)
        out_t = torch.clamp(model_no_gated_skips(d_t), 0.0, 1.0)
...
print("\n" + "="*80)
print("REZULTAT EVALUACIJE: ABLACIJA 7 (w/o Gated Skip Connections)")
print("="*80)
