# ==============================================================================
# ABLACIJA 10: BEZ CONTRAST COLOR RECOVERY (w/o Contrast Color Recovery - CCR)
...
# ==============================================================================
# ARHITEKTURA MODELA (BEZ CONTRAST COLOR RECOVERY MODULA)
# ==============================================================================
...
# GLAVNI MODEL ZA ABLACIJU 10: BEZ CONTRAST COLOR RECOVERY (standardna direktna konvolucija umesto CCR)
class Restauracija_NoCCR(nn.Module):
    def __init__(self, in_channels: int = 3, out_channels: int = 3, base_ch: int = 32):
        ...
        # BEZ CCR: Standardna lokalna konvolucija bez globalnog adaptivnog gejtinga i biasa
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # BEZ CCR: Direktno sabiranje bez globalnog pojačanja (gain) i pomaka (bias)
        residual = self.direct_conv(fused_out)
...
# ==============================================================================
model_no_ccr = Restauracija_NoCCR(base_ch=32).to(device)
...
CKPT_SEPIA = os.path.join(DIR_ABLACIJA_CKPT, "ablation_no_ccr_sepia25ep.pth")
if os.path.exists(CKPT_SEPIA):
    print(f"✓ [Keš] Učitavam postojeći checkpoint sa 25 epoha Sepia: {CKPT_SEPIA}")
    model_no_ccr.load_state_dict(torch.load(CKPT_SEPIA, map_location=device))
else:
    print(f"\n=======================================================")
    print(f" FAZA 1: Trening bez CCR modula ({EPOCHS_SEPIA} epoha na Sepia)")
    print(f"=======================================================")
    opt_pretrain = torch.optim.AdamW(model_no_ccr.parameters(), lr=LR_PRETRAIN, weight_decay=1e-4)
    ...
    torch.save(model_no_ccr.state_dict(), CKPT_SEPIA)
    print(f"✓ Sačuvan bazni Sepia checkpoint: {CKPT_SEPIA}")

# FAZA 2: 5 EPOHA FINE-TUNINGA NA TRENING SKUPU
CKPT_FINAL = os.path.join(DIR_ABLACIJA_CKPT, "ablation_no_ccr_final.pth")
if os.path.exists(CKPT_FINAL):
    print(f"✓ [Keš] Učitavam finalni dotrenirani model: {CKPT_FINAL}")
    model_no_ccr.load_state_dict(torch.load(CKPT_FINAL, map_location=device))
else:
    print(f"\n=======================================================")
    print(f" FAZA 2: Fine-Tuning ({EPOCHS_FT} epoha na ciljnom datasetu)")
    print(f"=======================================================")
    opt_ft = torch.optim.AdamW(model_no_ccr.parameters(), lr=LR_FINETUNE, weight_decay=1e-4)
    ...
    torch.save(model_no_ccr.state_dict(), CKPT_FINAL)
    print(f"✓ Sačuvan finalni model: {CKPT_FINAL}")

# FAZA 3: EVALUACIJA NA VALIDACIJI
print(f"\n[INFO] Evaluacija modela (w/o Contrast Color Recovery) na validaciji...")
model_no_ccr.eval()
...
with torch.no_grad():
    for fname in val_files:
        ...
        out_t = torch.clamp(model_no_ccr(d_t), 0.0, 1.0)
...
rezultati = [[
    "10. w/o Contrast Color Recovery (CCR)",
    f"{p_m:.2f} ± {p_sd:.2f}",
    f"{s_m:.4f} ± {s_sd:.4f}",
    f"{l_m:.4f} ± {l_sd:.4f}"
]]
...
print("\n" + "="*80)
print("REZULTAT EVALUACIJE: ABLACIJA 10 (w/o Contrast Color Recovery)")
print("="*80)
print(tabulate(rezultati, headers=["Konfiguracija", "PSNR [↑]", "SSIM [↑]", "LPIPS [↓]"], tablefmt="fancy_grid"))

csv_out = os.path.join(DRIVE_PROJECT_DIR, "rezultat_ablacija_10_no_ccr.csv")
