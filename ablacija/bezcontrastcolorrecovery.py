...
# ==============================================================================
# MODEL: ABLACIJA 10 (BEZ CONTRAST COLOR RECOVERY MODULA)
# ==============================================================================
...
class Restauracija_NoCCR(nn.Module):
    def __init__(self, in_channels: int = 3, out_channels: int = 3, base_ch: int = 32):
        ...
        # BEZ CCR: Standardna lokalna konvolucija umesto ContrastColorRecovery
    ...
    def forward(self, x: Tensor) -> Tensor | dict[str, Tensor]:
        ...
        # BEZ CCR: Direktna lokalna konvolucija
        residual = self.direct_conv(fused_out)
        out = torch.clamp(input_img + residual, 0.0, 1.0)
    ...
    for ep in range(EPOCHS_SEPIA):
        ...
        for batch_idx, (d_t, c_t, _) in enumerate(sepia_loader):
            ...
            if (batch_idx + 1) % ACCUMULATION_STEPS == 0 or (batch_idx + 1) == len(sepia_loader):
                torch.nn.utils.clip_grad_norm_(model_no_ccr.parameters(), max_norm=1.0)
                opt_pretrain.step()
                opt_pretrain.zero_grad()
...
print(f"\n[INFO] Evaluacija modela (w/o Contrast Color Recovery)...")
model_no_ccr.eval()
