# ==============================================================================
# ABLACIJA 1: BEZ SPATIAL ENCODER GRANE (w/o Spatial Encoder Stream) - FINALNO
...
# ==============================================================================
# MODEL: ABLACIJA 1 (BEZ SPATIAL ENCODER-A)
# ==============================================================================
...
class Restauracija_NoSpatial(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor | dict[str, Tensor]:
        ...
        sk4 = self.skip_refine4(self.skip_gate4(sp4))
        sk3 = self.skip_refine3(self.skip_gate3(sp3))
        sk2 = self.skip_refine2(self.skip_gate2(sp2))
        sk1 = self.skip_refine1(self.skip_gate1(sp1))
...
CKPT_SEPIA = os.path.join(DIR_ABLACIJA_CKPT, "ablation1_no_spatial_sepia25ep_fixed.pth")
CKPT_FINAL = os.path.join(DIR_ABLACIJA_CKPT, "ablation1_no_spatial_final_fixed.pth")
...
if os.path.exists(CKPT_SEPIA):
    ...
    print(f"=======================================================")
    print(f" FAZA 1: Trening bez Spatial grane ({TARGET_EPOCHS_SEPIA} epoha na Sepia)")
    print(f"=======================================================")
    opt_pretrain = torch.optim.AdamW(model_no_spatial.parameters(), lr=LR_PRETRAIN, weight_decay=1e-4)
    sched_pretrain = torch.optim.lr_scheduler.CosineAnnealingLR(opt_pretrain, T_max=TARGET_EPOCHS_SEPIA, eta_min=1e-6)

    for ep in range(TARGET_EPOCHS_SEPIA):
        ...
        for batch_idx, (d_t, c_t, _) in enumerate(sepia_loader):
            ...
            if (batch_idx + 1) % ACCUMULATION_STEPS == 0 or (batch_idx + 1) == len(sepia_loader):
                torch.nn.utils.clip_grad_norm_(model_no_spatial.parameters(), max_norm=1.0)
                opt_pretrain.step()
                opt_pretrain.zero_grad()
        ...
        sched_pretrain.step()
        print(f" [Sepia Epoha {ep+1:02d}/{TARGET_EPOCHS_SEPIA}] Loss: {running_loss/len(sepia_loader):.4f} | LR: {sched_pretrain.get_last_lr()[0]:.6f}")
...
if os.path.exists(CKPT_FINAL):
    ...
    print(f"\n=======================================================")
    print(f" FAZA 2: Fine-Tuning ({TARGET_EPOCHS_FT} epoha na ciljnom datasetu)")
    print(f"=======================================================")
    ...
    sched_ft = torch.optim.lr_scheduler.CosineAnnealingLR(opt_ft, T_max=TARGET_EPOCHS_FT, eta_min=1e-6)

    for ep in range(TARGET_EPOCHS_FT):
        model_no_spatial.train()
        running_loss = 0.0
        opt_ft.zero_grad()
        ...
        sched_ft.step()
        print(f" [Fine-Tune Epoha {ep+1:02d}/{TARGET_EPOCHS_FT}] Loss: {running_loss/len(train_loader):.4f}")
...
print(f"\n[INFO] Evaluacija modela (w/o Spatial Encoder)...")
...
with torch.no_grad():
    for d_t, c_t, _ in val_loader:
        d_t, c_t = d_t.to(device), c_t.to(device)
        out_t = torch.clamp(model_no_spatial(d_t), 0.0, 1.0)
...
print("\n" + "="*80)
print("REZULTAT EVALUACIJE: ABLACIJA 1 (w/o Spatial Encoder)")
print("="*80)
print(tabulate(rezultati, headers=["Konfiguracija", "PSNR [↑]", "SSIM [↑]", "LPIPS [↓]"], tablefmt="fancy_grid"))

csv_out = os.path.join(DRIVE_PROJECT_DIR, "rezultat_ablacija_1_no_spatial_fixed.csv")
