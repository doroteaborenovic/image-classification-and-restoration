...
def pronadji_sepia_foldere(base_dir, fallback_clean_dir):
    ...
    if os.path.exists(train_dir):
        ...
        if '0' in subdirs:
            # Ako je 0 degradirana sepia, a clean je u glavnom trening skupu
            return fallback_clean_dir, os.path.join(train_dir, '0')
    # 3. Direktno /content/dataset_sepia/train/0
...
class PairedDataset(Dataset):
    def __init__(self, clean_dir, degraded_dir, img_size=256, train=False):
        ...
        if len(self.files) == 0:
            # Fallback ako se poklapaju po nazivu bez ekstenzije
...
# GLAVNI ABLIRANI MODEL: SAMO SPECTRAL GRANA
...
CKPT_SEPIA = os.path.join(DIR_ABLACIJA_CKPT, "ablation_no_spatial_sepia25ep.pth")
if os.path.exists(CKPT_SEPIA):
    ...
    print(f"\n=======================================================")
    print(f" FAZA 1: Trening bez Spatial grane ({EPOCHS_SEPIA} epoha na Sepia)")
    print(f"=======================================================")
...
CKPT_FINAL = os.path.join(DIR_ABLACIJA_CKPT, "ablation_no_spatial_final.pth")
...
csv_out = os.path.join(DRIVE_PROJECT_DIR, "rezultat_ablacija_1_no_spatial.csv")
